...
// 🙋 上面這樣寫太繁瑣了，該如何建立通用版本呢 ?

const isMultipleOf = (n1: number) => (n2: number) =>
  n2 % n1 === 0

// 🙋 對照文件 predicate - utils - Predicate (interface)
// 請找出在 isMultipleOf(4) 裡面的 A

// 📝 把 predicate 組合起來完成閏年定義

const isLeapYearV1 = pipe(
  isMultipleOf(4),
  P.and(P.not(isMultipleOf(100))),
  P.or(isMultipleOf(400))
)
...
//  請找出在 P.or(isMultipleOf(400)) 裡面的 A、B、f、predicate
